...
FocusTrail is organized by responsibility. Frontend UI lives in `src/`, local API and persistence code lives in `server/`, and the optional macOS activity collector lives in `scripts/`.
